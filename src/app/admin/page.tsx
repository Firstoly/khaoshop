...
import { Store, ShoppingBag, Users, TrendingUp, CheckCircle2, Clock, Package, ExternalLink, ArrowRight, ChevronRight } from 'lucide-react'
...
export default async function AdminPage() {
  ...
  const stats = [
    { label: 'ร้านค้าในระบบ',          value: shops.length,              unit: 'ร้าน',    icon: Store,      bg: 'bg-orange-50',  text: 'text-orange-500' },
    { label: 'ผู้ใช้ทั้งหมด',           value: totalUsers,                unit: 'คน',     icon: Users,      bg: 'bg-blue-50',    text: 'text-blue-500'   },
    { label: 'ออเดอร์วันนี้ทุกร้าน',   value: todayOrdersAll,            unit: 'รายการ', icon: ShoppingBag,bg: 'bg-violet-50',  text: 'text-violet-500' },
    { label: 'รายได้รวม (ยืนยันแล้ว)', value: formatPrice(totalRevenue), unit: '',        icon: TrendingUp, bg: 'bg-emerald-50', text: 'text-emerald-600'},
  ]
    ...
    <div className="space-y-6 max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/admin/users"
          className="card-base p-4 flex items-center gap-4 hover:shadow-card-hover transition-all duration-200 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card-base p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-400">ทุกร้านรวม</span>
          </div>
          <div className="space-y-2">
            {recentOrders.length === 0 ? (
              ...
              <div key={order.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-violet-50 transition-colors cursor-default">
                ...
                <div className="text-right shrink-0">
                  ...
                  {order.paymentStatus === 'VERIFIED'
                    ? <span className="text-[10px] text-emerald-600 flex items-center gap-0.5 justify-end"><CheckCircle2 className="w-3 h-3" />ยืนยัน</span>
                    : order.paymentStatus === 'PAID'
                    ? <span className="text-[10px] text-blue-500 flex items-center gap-0.5 justify-end"><Clock className="w-3 h-3" />รอตรวจ</span>
        ...
        <div className="card-base p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/shops" className="text-xs text-violet-500 hover:text-violet-600 font-medium transition-colors">
              ดูทั้งหมด →
            </Link>
          </div>
          <div className="space-y-3">
            {shops.slice(0, 7).map(shop => (
              <div key={shop.id} className="flex items-center gap-3 group">
            ...
            {shops.length > 7 && (
              <Link href="/admin/shops"
                className="flex items-center gap-1 text-xs text-violet-400 hover:text-violet-600 font-medium pt-1 transition-colors">
                <ChevronRight className="w-3 h-3" />
                ดูอีก {shops.length - 7} ร้าน
              </Link>
            )}
          </div>
        </div>
